...
./start.sh dag      # 仅启动 DAG 可视化
./start.sh stop     # 停止全部服务
./start.sh status   # 查看服务状态
```

**关键说明**：Streamlit 页面内的 DAG 可视化 iframe 必须通过 **IP:PORT** 访问（不能用 `localhost`），`start.sh` 会自动探测本机 IP 并设置 `DAG_APP_URL` 环境变量。如需手动指定：

```bash
DAG_APP_URL=http://172.18.40.105:3001 ./start.sh
```
...
```bash
# 1. 后端（FastAPI，端口 8005）
cd /home/shijialin/paperworking/workflow_new/productinsight-agent
python -m uvicorn backend.app.main:app --reload --port 8005 --host 0.0.0.0

# 2. DAG 可视化（ReactFlow，生产静态服务器，端口 3001）
# 必须用 node server.js（而非 npm run dev），否则 iframe 无法正常加载
cd frontend/dag-app
node server.js

# 3. Streamlit 前端（端口 8501）
# 必须设置 DAG_APP_URL 指向可从浏览器访问的 DAG App 地址
...
> 需要先归档的旧脚本 `reverse_proxy.py` 已废弃，不建议使用。

使用 Cloudflare Tunnel 将本地服务暴露到公网（无需登录，每次重启 URL 会变化）：

```bash
# 1. 安装 cloudflared（从 GitHub 下载，无需登录）
curl -L -o ~/cloudflared \
  "https://github.com/cloudflare/cloudflare/releases/latest/download/cloudflared-linux-amd64"
chmod +x ~/cloudflared

# 2. 先用 start.sh 启动全部服务
./start.sh

# 3. 启动隧道，暴露 Streamlit 端口（前后端 + DAG 均由 Streamlit 页面内嵌）
~/cloudflared tunnel --url http://localhost:8501
# 输出中会显示公网 URL，如 https://xxxx.trycloudflare.com
```

> 评委直接打开隧道 URL 即可访问完整 Demo（Streamlit 页面内嵌了 DAG iframe）。
...
### DAG 可视化不显示（iframe 空白）

这是最常见的问题：**iframe 的 `src` 必须是可以从浏览器访问的地址**（不能用 `localhost`）。

```bash
# 查看当前 DAG iframe URL 是否正确
./start.sh status
# 输出中的 "DAG iframe URL" 就是 Streamlit iframe 实际使用的地址
...
```bash
# 1. 确认 DAG App 在服务器上正常运行
curl http://localhost:3001/

# 2. 确认 DAG App 对外可访问（从本机 curl 服务器 IP）
curl http://172.18.40.105:3001/

# 3. 确认 Streamlit 启动时设置了 DAG_APP_URL
grep -i "DAG_APP" /tmp/productinsight/streamlit.log | head -5
```
...
│   ├── views/              # 前端页面模块
│   └── dag-app/            # ReactFlow DAG 可视化（node server.js 启动）
├── scripts/
│   ├── start.sh           # ★ 一键启动（仅此脚本在根目录）
│   ├── seed_golden_demo.py  # 预置演示数据
│   ├── test_golden_demo.py  # 验证演示数据完整性
│   ├── test_e2e_*.py        # E2E 测试套件
│   ├── test_generalization_*.py  # 泛化能力测试
│   ├── record_demo.py       # 自动化录屏
│   ├── pack_for_submission.py  # 打包提交
│   ├── verify_demo.py       # 答辩前检查脚本
│   └── archive/             # 废弃/调试脚本（勿用）
├── data/
│   ├── reports/             # 报告输出
...
> **警告**：`scripts/archive/` 下的脚本均为废弃代码，请勿使用。活跃脚本仅限 `scripts/` 根目录下带 `#!/` 或 `"""` 开头注释的 Python 脚本，以及根目录的 `./start.sh`。

---

## 打包提交

使用项目内置打包脚本生成干净 zip 包：

```bash
cd /home/shijialin/paperworking/workflow_new
python3 productinsight-agent/scripts/pack_for_submission.py
```

脚本自动排除：`.env`（真实 API Key）、`productinsight.db`（运行时数据库）、内部开发文档、`docs/`（内部开发记录）、测试脚本和报告输出文件。保留 `.env.example` 模板。
