...
from frontend.common.api import API_BASE
...
# Handle ?viewer= query param: open Report Viewer directly
if st.query_params.get("viewer"):
    viewer_rid = st.query_params["viewer"]
    st.session_state["current_page_zh"] = "Report Viewer"
    st.session_state["report_viewer_run_id"] = viewer_rid
...
def render_review_center_action_row(run_id: str, key_prefix: str):
    ...
    with action_cols[1]:
        if st.button("View Evidence", key=f"{key_prefix}_view_evidence_{run_id}", use_container_width=True):
            st.session_state["evidence_run_id"] = run_id
...
# Replaced inline block with views/analysis_flow.py (has DAG iframe + running center)
# ---------------------------------------------------------------------------

if page == "AnalysisFlow":
    from frontend.views.analysis_flow import render_analysis_flow
...
elif page == "ReportViewer":
    viewer_run_id = st.session_state.get("report_viewer_run_id") or run_id
    if not viewer_run_id:
        st.warning("No run_id specified for the report viewer.")
        st.stop()
    from frontend.views.report_viewer import render_report_viewer
    render_report_viewer(viewer_run_id)
                    ...
                    with st.container():
                            ...
                            with bd:
                                    if st.button("Evidence", key=f"ev_{pid}"):
                                            st.session_state["selected_project_id"] = pid
                                            st.session_state["evidence_run_id"] = None  # clear stale run_id, use project-level query
                                            goto_page("Evidence Hub")
                            st.divider()
...
# ---------------------------------------------------------------------------
    ...
    # Priority: evidence_run_id (set by report viewer) > selected_run_id > run_id
    evidence_run_id = st.session_state.get("evidence_run_id") or st.session_state.get("selected_run_id") or run_id
    if evidence_run_id:
        st.caption(f"Showing evidence for run: `{evidence_run_id}`")
        evidence = get_json(f"/api/runs/{evidence_run_id}/evidence", [])
            ...
            with tab_overview:
                    ...
                    with qa_col1:
                            if st.button("Evidence Hub", key=f"ws_overview_evidence_{active_run_id}", use_container_width=True):
                                    st.session_state["evidence_run_id"] = active_run_id
                                    goto_page("Evidence Hub")
                    with qa_col2:
...
# Replaced inline block with views/research_plan_page.py (has full plan editing)
# ---------------------------------------------------------------------------

elif page == "ResearchPlan":
    from frontend.views.research_plan_page import render_research_plan_page
    render_research_plan_page()
